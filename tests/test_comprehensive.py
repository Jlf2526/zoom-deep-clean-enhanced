...
class TestSecurityValidation(unittest.TestCase):
    ...
    def test_operation_signature_generation(self):
        ...
        signature = self.security_validator.generate_operation_signature(operation, path)
        self.assertIsInstance(signature, str)
        self.assertTrue(len(signature) > 0)
        
        # Verify signature
        self.assertTrue(
            self.security_validator.verify_operation_signature(operation, path, signature)
        )
        
        # Test with wrong signature
        wrong_signature = "wrong_signature"
        self.assertFalse(
            self.security_validator.verify_operation_signature(operation, path, wrong_signature)
        )
...
class TestSystemFingerprintAnalyzer(unittest.TestCase):
    ...
    def test_hardware_identifier_extraction(self, mock_subprocess):
        ...
        mock_subprocess.return_value.stdout = '"IOPlatformUUID" = "12345678-1234-1234-1234-123456789ABC"'
        
        identifiers = self.analyzer._get_hardware_identifiers()
        
        self.assertIn('system_uuid', identifiers)
        self.assertEqual(identifiers['system_uuid'], '12345678-1234-1234-1234-123456789ABC')
...
class TestZoomDeepCleanerEnhanced(unittest.TestCase):
    ...
    def test_statistics_tracking(self):
        ...
        expected_keys = [
            'files_removed', 'directories_removed', 'processes_killed',
            'vm_services_stopped', 'keychain_entries_removed', 'files_backed_up',
            'errors', 'warnings', 'security_violations', 'remaining_files_found'
        ]
