...
class TestPlatformDetector(unittest.TestCase):
    ...
    def test_get_platform_info(self):
        ...
        required_keys = ["system", "version", "architecture", "python_version", "supported"]
...
class TestWindowsZoomCleaner(unittest.TestCase):
    ...
    def test_windows_zoom_cleaner_initialization_success(self, mock_winreg, mock_system):
    ...
    def test_windows_clean_zoom_dry_run(self, mock_winreg, mock_system):
        ...
        expected_keys = ["processes_terminated", "files_removed", "registry_keys_removed", "services_stopped", "errors"]
        for key in expected_keys:
    ...
    def test_windows_terminate_zoom_processes_dry_run(self, mock_subprocess, mock_winreg, mock_system):
    ...
    @patch('zoom_deep_clean.cross_platform_support.winreg')
    @patch('subprocess.run')
    def test_windows_terminate_zoom_processes_actual(self, mock_subprocess, mock_winreg, mock_system):
    ...
    def test_windows_clean_registry_dry_run(self, mock_winreg, mock_system):
        ...
        mock_winreg.DeleteKeyEx.assert_not_called() if hasattr(mock_winreg, 'DeleteKeyEx') else None
    ...
    @patch('shutil.rmtree')
    @patch('os.remove')
    def test_windows_remove_zoom_files_dry_run(self, mock_remove, mock_rmtree, mock_exists, mock_winreg, mock_system):
    ...
    @patch('zoom_deep_clean.cross_platform_support.winreg')
    @patch('subprocess.run')
    def test_windows_stop_zoom_services_dry_run(self, mock_subprocess, mock_winreg, mock_system):
    ...
    def test_windows_clean_zoom_exception_handling(self, mock_winreg, mock_system):
        ...
        cleaner._terminate_zoom_processes = MagicMock(side_effect=Exception("Test exception"))
...
class TestLinuxZoomCleaner(unittest.TestCase):
    ...
    def test_linux_clean_zoom_dry_run(self, mock_system):
        ...
        # Verify results structure
        expected_keys = ["processes_terminated", "files_removed", "packages_removed", "services_stopped", "errors"]
        for key in expected_keys:
            self.assertIn(key, results)
    ...
    @patch('shutil.rmtree')
    @patch('os.remove')
    def test_linux_remove_zoom_files_dry_run(self, mock_remove, mock_rmtree, mock_exists, mock_system):
