...
class PerformanceTracker:
    ...
    def run_timed_tests(self) -> Dict[str, float]:
        ...
        try:
            subprocess.run([
                sys.executable, "-m", "flake8", "zoom_deep_clean", 
                "--count", "--statistics"
    ...
    def _analyze_trends(self, historical: List[Dict], current: Dict) -> Dict:
        ...
        for metric in ["test_time", "lint_time", "security_time", "total_time"]:
            ...
            if len(values) >= 3:
                ...
                avg_older = statistics.mean(values[:-3]) if len(values) > 3 else avg_recent
                
                trend_percent = ((avg_recent - avg_older) / avg_older * 100) if avg_older > 0 else 0
    ...
    def generate_report(self, analysis: Dict) -> str:
        ...
        for metric, data in analysis["threshold_analysis"].items():
            ...
            report.append(f"  {emoji} {metric}: {data['current']:.1f}s ({data['percentage']:.1f}% of {data['threshold']}s)")
        report.append("")
        ...
        if analysis["trend_analysis"]:
            ...
            for metric, data in analysis["trend_analysis"].items():
                ...
                report.append(f"  {emoji} {metric}: {data['trend_direction']} ({data['trend_percent']:+.1f}%)")
            report.append("")
...
def main():
    ...
    parser.add_argument("--metrics-file", default="performance_metrics.json", 
