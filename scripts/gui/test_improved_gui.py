...
def test_gui_improvements():
    ...
    try:
        ...
        def add_test_messages():
            ...
            for i in range(50):
                app.log_message(
                    f"Test info message {i+1} - This is a longer message to test text wrapping and scrolling performance",
                    "info",
                )
                if i % 10 == 0:
            ...
            app.log_message(
                "✅ Keyboard shortcuts (Cmd+A, Cmd+C, Cmd+Shift+K)", "success"
            )
        ...
        app.log_message(
            "✨ Testing enhanced scrolling and performance features", "info"
        )
        app.log_message("📝 Right-click in this area for context menu", "info")
        app.log_message("🖱️ Use mouse wheel to scroll", "info")
        app.log_message(
            "⌨️ Try keyboard shortcuts: Cmd+A (select all), Cmd+C (copy), Cmd+Shift+K (clear)",
            "info",
        )
