...
class AsyncFileScanner:
    ...
    def __init__(self, logger: logging.Logger, max_workers: int = 8):
        ...
        self.zoom_patterns = [
            '*zoom*', '*Zoom*', '*ZOOM*',
            '*us.zoom*', '*zoom.us*',
            '*zoomopener*', '*ZoomOpener*',
            '*zoomphone*', '*ZoomPhone*',
            '*zoomoutlook*', '*ZoomOutlook*',
            '*zoomrooms*', '*ZoomRooms*'
        ]
        
        # Directories to exclude for performance
        self.excluded_dirs = {
            '.Trash', 'Trash', 'Library/Caches',
            'Application Support/MobileSync',
            'Time Machine Backups', '.git', '.svn',
    ...
    async def scan_directories_parallel(self, directories: List[str], 
...
class PerformanceOptimizer:
    ...
    async def optimized_file_search(self, search_locations: List[str],
