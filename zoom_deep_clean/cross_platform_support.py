...
class WindowsZoomCleaner:
    ...
    def _terminate_zoom_processes(self) -> int:
        ...
        zoom_processes = [
            'Zoom.exe', 'ZoomLauncher.exe', 'ZoomPhone.exe',
            'ZoomChat.exe', 'ZoomClips.exe', 'ZoomPresence.exe'
        ]
    ...
    def _terminate_zoom_processes(self) -> int:
        ...
        try:
            if self.dry_run:
                ...
                result = subprocess.run(['pkill', '-f', 'zoom'], 
                                      capture_output=True, text=True)
                if result.returncode == 0:
        ...
        for list_cmd, remove_cmd in package_managers:
            try:
                ...
                if result.returncode == 0:
                    ...
                    if 'zoom' in packages:
                        if self.dry_run:
                            self.logger.info(f"DRY RUN: Would remove Zoom package with {remove_cmd[0]}")
                            removed += 1
                        else:
                            # Remove zoom package
                            remove_result = subprocess.run(remove_cmd + ['zoom'],
                                                         capture_output=True, text=True)
...
class CrossPlatformZoomCleaner:
    ...
    def run_cross_platform_cleanup(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"🌍 Running cleanup on {platform_info['system']} {platform_info['version']}")
        ...
        if self.platform_detector.platform == 'darwin':
            ...
            results['cleanup_results'] = self.platform_cleaner.clean_linux_zoom() if self.platform_detector.platform == 'linux' else self.platform_cleaner.clean_windows_zoom()
