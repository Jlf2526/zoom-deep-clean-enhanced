...
class ZoomDeepCleanerEnhanced:
    ...
    def _setup_logging(self) -> None:
        ...
        self.logger.info(f"Zoom Deep Clean Enhanced v2.2.0 (VM-Aware & System-Wide) by PHLthy215 started")
    ...
    def _validate_environment(self) -> None:
        ...
        if os.geteuid() == 0:
            self.logger.warning("Running as root - this may cause permission issues with user files")
    ...
    def _run_command(self, cmd_args: Union[str, List[str]], description: str = "", 
                    require_sudo: bool = False, timeout: int = 30) -> Tuple[bool, str]:
        ...
        if isinstance(cmd_args, str):
            ...
            self.logger.warning(f"String command converted to list for security: {cmd_args}")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.warning(f"Command failed: {' '.join(cmd_args)} - {error_msg}")
                self.cleanup_stats["warnings"] += 1
                return False, error_msg
                
        except subprocess.TimeoutExpired:
            self.logger.error(f"Command timed out after {timeout}s: {' '.join(cmd_args)}")
    ...
    def _verify_process_cleanup(self) -> bool:
        ...
        success, output = self._run_command(
            ["pgrep", "-f", "[Zz]oom"],
            "Checking for remaining Zoom processes"
        )
        
        if success and output.strip():
            remaining_processes = output.strip().split('\n')
            self.logger.warning(f"Found {len(remaining_processes)} remaining Zoom processes:")
    ...
    def stop_zoom_processes(self) -> None:
        ...
        if not self.dry_run:
            ...
            self.logger.info(f"⏳ Waiting {grace_period}s for processes to terminate gracefully...")
    ...
    def _verify_zoom_file(self, path: str) -> bool:
        ...
        try:
            ...
            zoom_keywords = ['zoom', 'us.zoom', 'zoomphone', 'zoomclips', 'zoomchat', 'zoompresence']
    ...
    def _remove_path(self, path: str, description: str = "", force: bool = False) -> bool:
        ...
        if not self._backup_path(validated_path):
            self.logger.warning(f"Backup failed for {validated_path}, proceeding with removal")
        
        try:
            if os.path.isdir(validated_path):
                shutil.rmtree(validated_path)
                self.cleanup_stats["directories_removed"] += 1
                self.logger.info(f"✅ Removed directory: {description or validated_path}")
            elif os.path.isfile(validated_path):
                os.remove(validated_path)
                self.cleanup_stats["files_removed"] += 1
                self.logger.info(f"✅ Removed file: {description or validated_path}")
            else:
                self.logger.warning(f"Path exists but is neither file nor directory: {validated_path}")
                return False
    ...
    def remove_group_containers(self) -> None:
        ...
        for container in group_containers:
            self._remove_path(container, f"Group container: {os.path.basename(container)}")
    ...
    def comprehensive_file_search(self) -> List[str]:
        ...
        self.logger.info("🔍 Performing comprehensive search for remaining Zoom files...")
        ...
        for location in search_locations:
            ...
            success, output = self._run_command(
                ["find", location, "-iname", "*zoom*", "-type", "f"],
            ...
            if success and output.strip():
                found_files = [f.strip() for f in output.strip().split('\n') if f.strip()]
    ...
    def run_advanced_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.cleanup_stats["system_identifiers_detected"] = uuid_results["total_identifiers"]
            self.cleanup_stats["advanced_features_executed"] += 1
            
            # 4. Hostname Reset (if enabled)
            if self.reset_hostname:
                self.logger.info("=" * 60)
                hostname_results = self.advanced_features.reset_hostname(self.new_hostname)
                advanced_results["hostname_reset"] = hostname_results
                self.cleanup_stats["hostname_reset_success"] = hostname_results["success"]
                self.cleanup_stats["advanced_features_executed"] += 1
            ...
            if self.enable_mac_spoofing:
                ...
                if "interfaces_spoofed" in mac_results:
                    self.cleanup_stats["mac_addresses_spoofed"] = len(mac_results["interfaces_spoofed"])
                self.cleanup_stats["advanced_features_executed"] += 1
        ...
        try:
            ...
            if success:
                for line in output.split('\n'):
                    if any(keyword in line for keyword in ["Hardware UUID", "Serial Number", "Model Identifier"]):
    ...
    def save_report(self, report: Dict[str, Any]) -> None:
        ...
        try:
            report_file = self._validate_path(os.path.expanduser("~/Documents/zoom_cleanup_enhanced_report.json"))
    ...
    def run_deep_clean(self) -> bool:
        ...
        try:
            self.logger.info("🔥 ZOOM DEEP CLEAN ENHANCED - VM-Aware & System-Wide v2.2.0 by PHLthy215")
            self.logger.info("=" * 80)
            
            # Check sudo access for system operations
            if not self._check_sudo_access() and not self.dry_run:
                self.logger.warning("Sudo access not available - some system-level cleanup may fail")
            ...
            if remaining_files:
                self.logger.warning(f"⚠️ Found {len(remaining_files)} remaining Zoom files:")
                for file_path in remaining_files[:10]:  # Show first 10
                    self.logger.warning(f"   📄 {file_path}")
                if len(remaining_files) > 10:
                    self.logger.warning(f"   ... and {len(remaining_files) - 10} more files")
            
            self.show_hardware_info()
            
            # Generate and save report
            report = self.generate_report()
            report["advanced_features_results"] = advanced_results if 'advanced_results' in locals() else {}
            ...
            self.logger.info(f"   • Processes killed: {self.cleanup_stats['processes_killed']}")
            self.logger.info(f"   • VM services stopped: {self.cleanup_stats['vm_services_stopped']}")
            self.logger.info(f"   • Keychain entries removed: {self.cleanup_stats['keychain_entries_removed']}")
            self.logger.info(f"   • System locations cleaned: {self.cleanup_stats['system_locations_cleaned']}")
            self.logger.info(f"   • Files backed up: {self.cleanup_stats['files_backed_up']}")
            self.logger.info(f"   • Remaining files found: {self.cleanup_stats['remaining_files_found']}")
            
            # Advanced features statistics
            if self.enable_advanced_features:
                self.logger.info(f"   • Advanced features executed: {self.cleanup_stats['advanced_features_executed']}")
                self.logger.info(f"   • Keychain comprehensive scan: {self.cleanup_stats['keychain_comprehensive_scan']}")
                self.logger.info(f"   • MDM profiles detected: {self.cleanup_stats['mdm_profiles_detected']}")
                self.logger.info(f"   • System identifiers detected: {self.cleanup_stats['system_identifiers_detected']}")
                if self.reset_hostname:
                    self.logger.info(f"   • Hostname reset success: {self.cleanup_stats['hostname_reset_success']}")
                if self.enable_mac_spoofing:
                    self.logger.info(f"   • MAC addresses spoofed: {self.cleanup_stats['mac_addresses_spoofed']}")
            
            self.logger.info(f"   • Security violations: {self.cleanup_stats['security_violations']}")
            ...
            return (self.cleanup_stats['errors'] == 0 and 
                   self.cleanup_stats['security_violations'] == 0)
