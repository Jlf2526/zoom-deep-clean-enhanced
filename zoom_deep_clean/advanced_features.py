...
class AdvancedFeatures:
    ...
    def _run_command(self, cmd_args: List[str], description: str = "", 
        ...
        if self.dry_run and not description.startswith("Scanning") and not description.startswith("Detecting"):
    ...
    def scan_keychain_comprehensive(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.advanced_stats["keychain_entries_scanned"] = keychain_results["total_entries_scanned"]
            self.advanced_stats["keychain_zoom_entries_found"] = keychain_results["zoom_related_count"]
            
            self.logger.info(f"✅ Keychain scan complete: {keychain_results['zoom_related_count']} Zoom entries found")
    ...
    def detect_mdm_profiles(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if mdm_results["mdm_enrolled"]:
                self.logger.warning(f"⚠️ MDM enrollment detected with {mdm_results['total_profiles']} profiles")
            else:
                self.logger.info(f"✅ No MDM enrollment detected ({mdm_results['total_profiles']} local profiles)")
    ...
    def reset_hostname(self, new_hostname: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not new_hostname:
                ...
                adjectives = ["Swift", "Bright", "Quick", "Smart", "Fast", "Clean", "Fresh", "New"]
                nouns = ["Mac", "System", "Device", "Computer", "Machine", "Station", "Terminal"]
            ...
            if not self.dry_run:
                ...
                if hostname_results["success"]:
                    self.logger.info(f"✅ Hostname reset: {hostname_results['original_hostname']} → {new_hostname}")
                else:
                    self.logger.error("❌ Hostname reset failed")
            else:
                hostname_results["success"] = True
                self.logger.info(f"DRY RUN: Would reset hostname: {hostname_results['original_hostname']} → {new_hostname}")
    ...
    def spoof_mac_addresses(self) -> Dict[str, Any]:
        ...
        if not self.enable_mac_spoofing:
            self.logger.info("🔒 MAC address spoofing disabled (use --enable-mac-spoofing to enable)")
            ...
            if success:
                ...
                for line in output.split('\n'):
                    if line and not line.startswith('\t') and not line.startswith(' '):
                        ...
                        if interface_name not in ['lo0', 'gif0', 'stf0']:  # Skip loopback and tunnel interfaces
                ...
                vm_interfaces = [iface for iface in interfaces if any(vm in iface.lower() for vm in ['en', 'eth', 'vm'])]
                    ...
                    if interface in mac_results["original_macs"]:
                        ...
                        if not self.dry_run:
                            ...
                            if success1 and success2 and success3:
                                ...
                                self.logger.info(f"✅ MAC spoofed for {interface}: {mac_results['original_macs'][interface]} → {new_mac}")
                            else:
                                self.logger.error(f"❌ MAC spoofing failed for {interface}")
                        else:
                            mac_results["interfaces_spoofed"].append(interface)
                            self.logger.info(f"DRY RUN: Would spoof MAC for {interface}: {mac_results['original_macs'][interface]} → {new_mac}")
    ...
    def detect_system_uuids(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if success:
                ...
                for line in lines:
                    ...
                    if '"IOPlatformUUID"' in line:
                        ...
                        uuid_results["identifiers_found"].append({"type": "Hardware UUID", "value": uuid_val})
                    
                    elif '"IOPlatformSerialNumber"' in line:
                        serial = line.split('=')[1].strip().strip('"')
                        uuid_results["system_serial"] = serial
                        uuid_results["identifiers_found"].append({"type": "System Serial", "value": serial})
                    
                    elif '"board-id"' in line:
                        board_id = line.split('=')[1].strip().strip('"')
                        uuid_results["identifiers_found"].append({"type": "Board ID", "value": board_id})
                ...
                for line in output.split('\n'):
                    if 'Hardware UUID:' in line:
                        ...
                        if uuid_val and uuid_val != uuid_results["hardware_uuid"]:
                            ...
                            uuid_results["identifiers_found"].append({"type": "Platform UUID", "value": uuid_val})
                    
                    elif 'Serial Number (system):' in line:
                        serial = line.split(':')[1].strip()
                        if serial and serial != uuid_results["system_serial"]:
                            uuid_results["board_serial"] = serial
                            uuid_results["identifiers_found"].append({"type": "Board Serial", "value": serial})
            ...
            self.logger.info(f"✅ System identifier detection complete: {uuid_results['total_identifiers']} identifiers found")
            
            # Log identifiers (first few characters only for security)
            for identifier in uuid_results["identifiers_found"]:
                masked_value = identifier["value"][:8] + "..." if len(identifier["value"]) > 8 else identifier["value"]
