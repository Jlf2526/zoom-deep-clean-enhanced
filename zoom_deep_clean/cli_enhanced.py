...
Examples:
  ...
  %(prog)s --comprehensive-search    # Perform thorough file system search
    ...
    parser.add_argument(
        "--export-dry-run",
        metavar="FILE",
        help="Export dry-run operations to JSON file (dry-run mode only)",
    )
    ...
    try:
        ...
        # Export dry-run operations if requested
        if args.export_dry_run and args.dry_run:
            try:
                export_file = cleaner.export_dry_run_operations(args.export_dry_run)
                print(f"📋 Dry-run operations exported to: {export_file}")
            except Exception as e:
                print(f"❌ Failed to export dry-run operations: {e}")
        elif args.export_dry_run and not args.dry_run:
            print("⚠️  --export-dry-run can only be used with --dry-run mode")
