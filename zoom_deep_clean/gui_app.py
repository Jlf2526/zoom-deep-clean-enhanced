...
class ZoomCleanerGUI:
    ...
    def setup_variables(self):
        ...
        # Log file
        self.log_file_var = tk.StringVar(value=os.path.expanduser("~/Documents/zoom_deep_clean_enhanced.log"))
    ...
    def create_header(self, parent):
        ...
        version_label = ttk.Label(title_frame, text="v2.2.0 by PHLthy215", 
                                font=('SF Pro Display', 10), foreground=self.colors['secondary'])
    ...
    def create_basic_options(self, parent):
        ...
        # Dry Run
        ttk.Checkbutton(basic_frame, text="Preview Mode (Dry Run)", 
                       variable=self.dry_run_var,
                       command=self.on_dry_run_changed).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(basic_frame, text="Preview operations without making changes", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
        row += 1
        
        # Verbose
        ttk.Checkbutton(basic_frame, text="Detailed Logging", 
                       variable=self.verbose_var).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(basic_frame, text="Show detailed progress information", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
        row += 1
        
        # Backup
        ttk.Checkbutton(basic_frame, text="Create Backups", 
                       variable=self.backup_var).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(basic_frame, text="Backup files before removal (recommended)", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
        row += 1
        
        # VM Aware
        ttk.Checkbutton(basic_frame, text="VM-Aware Cleanup", 
                       variable=self.vm_aware_var).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(basic_frame, text="Stop VM services and clean VM-specific processes", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
        row += 1
        
        # System Reboot
        ttk.Checkbutton(basic_frame, text="Auto Reboot After Cleanup", 
                       variable=self.system_reboot_var).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(basic_frame, text="Automatically restart system when cleanup completes", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
    ...
    def create_advanced_options(self, parent):
        ...
        ttk.Checkbutton(advanced_frame, text="Enable Advanced Features", 
                       variable=self.advanced_features_var,
                       command=self.on_advanced_features_changed).grid(row=row, column=0, sticky=tk.W, pady=2)
        ttk.Label(advanced_frame, text="Keychain scan, MDM detection, UUID identification", 
                 font=('SF Pro Display', 10), foreground=self.colors['secondary']).grid(row=row, column=1, sticky=tk.W, padx=(10, 0))
        row += 1
        
        # Hostname Reset
        self.hostname_check = ttk.Checkbutton(advanced_frame, text="Reset System Hostname", 
        ...
        ttk.Label(hostname_frame, text="Custom name:", 
                 font=('SF Pro Display', 10)).grid(row=0, column=0, sticky=tk.W)
        self.hostname_entry = ttk.Entry(hostname_frame, textvariable=self.new_hostname_var, width=20)
        self.hostname_entry.grid(row=0, column=1, sticky=(tk.W, tk.E), padx=(5, 0))
        row += 1
        
        # MAC Spoofing
        self.mac_check = ttk.Checkbutton(advanced_frame, text="MAC Address Spoofing (VM Only)", 
                                       variable=self.mac_spoofing_var)
        self.mac_check.grid(row=row, column=0, sticky=tk.W, pady=2)
        
        mac_warning = ttk.Label(advanced_frame, text="⚠️ Use with caution - may affect network connectivity", 
                              font=('SF Pro Display', 10), foreground=self.colors['warning'])
    ...
    def create_log_options(self, parent):
        ...
        ttk.Button(log_entry_frame, text="Browse...", 
                  command=self.browse_log_file).grid(row=0, column=1)
    ...
    def create_action_buttons(self, parent):
        ...
        # Preview button
        self.preview_btn = ttk.Button(button_frame, text="🔍 Preview Cleanup", 
                                    command=self.preview_cleanup, style='Primary.TButton')
        self.preview_btn.grid(row=0, column=0, padx=(0, 10))
        
        # Run cleanup button
        self.run_btn = ttk.Button(button_frame, text="🔥 Run Cleanup", 
                                command=self.run_cleanup, style='Success.TButton')
        self.run_btn.grid(row=0, column=1, padx=(0, 10))
        
        # Stop button
        self.stop_btn = ttk.Button(button_frame, text="⏹ Stop", 
                                 command=self.stop_cleanup, style='Warning.TButton', state='disabled')
        self.stop_btn.grid(row=0, column=2, padx=(0, 10))
        
        # View logs button
        self.logs_btn = ttk.Button(button_frame, text="📋 View Logs", 
    ...
    def create_progress_section(self, parent):
        ...
        progress_frame.grid(row=5, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 15))
        ...
        self.output_text.tag_configure("timestamp", foreground=self.colors['secondary'], font=('Monaco', 9))
    ...
    def create_context_menu(self):
        ...
        self.context_menu.add_separator()
        self.context_menu.add_checkbutton(label="Auto-scroll", variable=tk.BooleanVar(value=True), 
    ...
    def cleanup_worker(self):
        ...
        try:
            ...
            hostname = self.new_hostname_var.get().strip() if self.reset_hostname_var.get() else None
...
Right-click in the output area for copy/paste options.
    ...
    def show_results_dialog(self, stats, success=True):
        """Show results dialog"""
        if success:
            messagebox.showinfo("Results", f"Operation completed with some warnings.\n\nCheck the log file for details.")
        else:
            messagebox.showerror("Results", f"Operation completed with errors.\n\nCheck the log file for details.")
