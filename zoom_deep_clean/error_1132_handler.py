...
class Error1132Handler:
            ...
            try:
                ...
                for port in critical_ports:
                    try:
                        ...
                        self.logger.error(f"❌ Error testing port {port} on {domain}: {e}")
                        results["all_passed"] = False
    ...
    def _check_firewall_rules(self) -> Dict[str, any]:
        ...
        try:
            result = subprocess.run(["sudo", "defaults", "read", 
        ...
        try:
            result = subprocess.run(["socketfilterfw", "--getglobalstate"],
                                  capture_output=True, text=True, timeout=10)
                ...
                for service in services:
                    if service and not service.startswith('*'):
                        ...
                        if "enabled: yes" in secure_proxy_result.stdout.lower():
                            results["system_proxies"][f"{service}_secure"] = "secure_web_proxy_enabled"
                        ...
                        try:
                            with open(log_file, 'r', encoding='utf-8', errors='ignore') as f:
                                ...
                                if "error 1132" in content.lower():
                                    self.logger.warning(f"❌ Found references to error 1132 in {log_file}")
                                    results["error_1132_found"] = True
                                    # Extract a snippet of the error context
                                    lines = content.split('\n')
                                    for i, line in enumerate(lines):
                                        if "error 1132" in line.lower() and i < len(lines) - 1:
                                            context = '\n'.join(lines[max(0, i-2):min(len(lines), i+3)])
                                            results["error_details"].append(f"File: {log_file}\n{context}")
                                            break
                        except Exception as e:
                            self.logger.warning(f"⚠️  Could not read log file {log_file}: {e}")
                            
                except Exception as e:
                    self.logger.warning(f"⚠️  Could not access log directory {expanded_path}: {e}")
    ...
    def _check_network_configuration(self) -> Dict[str, any]:
        ...
        try:
            result = subprocess.run(["ifconfig"], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                results["network_status"]["interfaces"] = len([line for line in result.stdout.split('\n') if line.startswith('\t')])
        except Exception as e:
    ...
    def _check_advanced_network_diagnostics(self) -> Dict[str, any]:
        ...
        try:
            result = subprocess.run(["ping", "-c", "4", "zoom.us"], 
    ...
    def _clear_dns_cache(self) -> bool:
        ...
        try:
            ...
            subprocess.run(["sudo", "killall", "-HUP", "mDNSResponder"], 
                          capture_output=True, check=True)
            subprocess.run(["sudo", "killall", "-HUP", "mDNSResponderHelper"], 
                          capture_output=True, check=True)
    ...
    def _reset_firewall_rules(self) -> bool:
        ...
        try:
            ...
            subprocess.run(["sudo", "defaults", "delete", 
                          "/Library/Preferences/com.apple.alf", "firewall"],
    ...
    def _clear_problematic_proxy_settings(self) -> bool:
        ...
        if found_proxies:
            self.logger.warning("⚠️  Found proxy settings that might interfere with Zoom:")
            for proxy in found_proxies:
                self.logger.warning(f"   • {proxy}")
            self.logger.info("💡 Recommendation: Check these proxy settings if issues persist")
            ...
            if result.returncode == 0:
                ...
                for service in services:
                    if service and not service.startswith('*'):
                        ...
                        subprocess.run(["networksetup", "-setairportpower", service, "off"], 
                                      capture_output=True, check=False, timeout=10)
                        time.sleep(2)
                        subprocess.run(["networksetup", "-setairportpower", service, "on"], 
                                      capture_output=True, check=False, timeout=10)
            ...
            if result.returncode == 0:
                ...
                for service in services:
                    if service and not service.startswith('*'):
                        ...
                        subprocess.run(["networksetup", "-setdnsservers", service, "8.8.8.8", "8.8.4.4"], 
                                      capture_output=True, check=False, timeout=10)
    ...
    def generate_error_1132_report(self, diagnostic_results: Dict[str, any]) -> str:
        ...
        if not connectivity.get("all_passed", True):
            ...
            report.append("2. Try changing your DNS servers to Google DNS (8.8.8.8, 8.8.4.4)")
        if not port_connectivity.get("all_passed", True):
            report.append("3. Check if your firewall or network is blocking Zoom ports (443, 8443, 3478)")
        if firewall.get("zoom_rules_found", False):
        ...
        report.append("10. Check if your organization has network policies blocking Zoom")
        report.append("")
