...
def test_all_features():
    ...
    try:
        ...
        print(f"   • Keychain comprehensive scan: {stats['keychain_comprehensive_scan']}")
        print(f"   • MDM profiles detected: {stats['mdm_profiles_detected']}")
        print(f"   • System identifiers detected: {stats['system_identifiers_detected']}")
        ...
        if cleaner.enable_advanced_features:
            ...
            print(f"   🔐 Keychain scan: {keychain_result['total_entries_scanned']} entries scanned")
            ...
            print(f"   🆔 UUID detection: {uuid_result['total_identifiers']} identifiers found")
            
            # Test hostname reset (dry-run)
            hostname_result = cleaner.advanced_features.reset_hostname("TestMac-DryRun")
            print(f"   🏷️ Hostname reset: {'Success' if hostname_result['success'] else 'Failed'}")
            
            # Test MAC spoofing (dry-run)
            mac_result = cleaner.advanced_features.spoof_mac_addresses()
            if mac_result.get('enabled', True):
                print(f"   🔄 MAC spoofing: {len(mac_result.get('interfaces_spoofed', []))} interfaces")
            else:
